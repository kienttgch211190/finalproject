...
const getAvailableTables = async (req, res) => {
  const { restaurantId, date, reservationTime, numGuests } = req.body;
  try {
    // Validate input
    if (!restaurantId || !date || !reservationTime || !numGuests) {
      return res.status(400).json({
        status: "Error",
        message: "Missing required fields: restaurantId, date, reservationTime, numGuests"
      });
    }
